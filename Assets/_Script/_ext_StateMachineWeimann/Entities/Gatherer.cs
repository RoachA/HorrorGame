...
public class Gatherer : MonoBehaviour
{
    ...
    private void Awake()
    {
        ...
        AddTransition(moveToSelected, search, StuckForOverASecond());
        AddTransition(moveToSelected, harvest, ReachedResource());
        AddTransition(harvest, search, TargetIsDepletedAndICanCarryMore());
        AddTransition(harvest, returnToStockpile, InventoryFull());
        AddTransition(returnToStockpile, placeResourcesInStockpile, ReachedStockpile());
        AddTransition(placeResourcesInStockpile, search, () => _gathered == 0);
        
        _stateMachine.AddAnyTransition(flee, () => enemyDetector.EnemyInRange);
        AddTransition(flee, search, () => enemyDetector.EnemyInRange == false);
